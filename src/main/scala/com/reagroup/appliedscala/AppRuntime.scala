...
class AppRuntime(
  ...
  private val fetchMovieController: FetchMovieController = {
    val fetchMovieService: FetchMovieService = new FetchMovieService(
  ...
  def routes: HttpApp[IO] =
    HttpApp((req: Request[IO]) =>
      appRoutes
        .openRoutes(req)
        .getOrElse(Response[IO](status = Status.NotFound))
    )
